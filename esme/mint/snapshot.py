...
class Snapshot:
    ...
    def is_diff(self, snap1, snap2):
        ...
        for i, ch in enumerate(self.channels):
            ...
            if (
                (self.channels_tol[i] is not None)
                and self.channels_track[i]
                and np.abs(snap1[ch][0] - snap2[ch][0]) > self.channels_tol[i]
            ):
...
class SnapshotDB:
        ...
        if y.__class__ is list:
            ...
            for yi in np.array(y):
                data = self.get_data_slice(
                    x, yi, beam_on=beam_on, calc_moments=calc_moments, start_inx=start_inx, stop_inx=stop_inx
                )
                data_slices.append(data)
        else:
            data_slices = self.get_data_slice(
                x, y, beam_on=beam_on, calc_moments=calc_moments, start_inx=start_inx, stop_inx=stop_inx
            )
        return data_slices
    ...
    def plot_orbit(self, section_id, legend_item=None, subtract_first=False, halve=False):
        ...
        x_orbits, y_orbits, x_bpm_names, y_bpm_names = self.get_orbit_dicts(
            section_id=section_id, legend_item=legend_item
        )
        ...
        plt.xticks(np.arange(len(bpm_names)), bpm_names)
    ...
    def plot_orbits(self, section_ids, legend_item=None, subtract_first=False, halve=False):
        ...
        for sec_id in section_ids:
            x_orbits, y_orbits, x_bpm_names, y_bpm_names = self.get_orbit_dicts(
                section_id=sec_id, legend_item=legend_item
            )
